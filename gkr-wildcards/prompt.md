...
9. Require contextual coherence for every generated subject, object, action, material, and setting—not only for creatures or vehicles. A canonical vocabulary item is not automatically appropriate in context. State whether a prop is worn, held, attached, operated, contained, nearby, or used as an explicit scale reference; do not imply that anatomy performs an impossible action or wears incompatible equipment. Technical and interior details must be visible from the selected viewpoint. Omit arbitrary props that do not support the subject, action, setting, scale, or focal idea.
...
- Treat an underscore-form token as a claim that the complete token exists in the authoritative vocabulary. Validate the unweighted inner token when weight syntax is present. Retrieval palettes guide selection but do not limit validity: an exact vocabulary item outside the initial palette remains valid, while a plausible-looking item absent from the vocabulary remains invalid.
...
- For a short literal phrase, prefer canonical coverage for at least half of its content words when faithful tags exist (at least one of two words and two of four words). This is a retrieval and review target, not permission to split a cohesive phrase or replace it with merely associated tags. Preserve unmatched words and attachment when the canonical vocabulary cannot express the complete meaning.
